// Firebase Function to promote a client
const { onCall, HttpsError  } = require("firebase-functions/v1/https");
const { initializeApp } = require("firebase-admin/app");
const { getAuth } = require("firebase-admin/auth");
const { getFirestore, FieldValue } = require("firebase-admin/firestore");
const sgMail = require("@sendgrid/mail");
// const functions = require("firebase-functions");
import * as functions from "firebase-functions";

sgMail.setApiKey(functions.config().sendgrid.api_key);

initializeApp();

/**
 * Promotes a client by creating a Firebase Auth user and linking it to the investor.
 * Expects: { uid, email } from the frontend.
 */
exports.promoteClient = onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new Error("Only authenticated users can call this.");
  }

  const callerUid = context.auth.uid;
  const db = getFirestore();
  const callerDoc = await db.collection("users").doc(callerUid).get();

  if (!callerDoc.exists || callerDoc.data().role !== "realtor") {
    throw new Error("Only realtors can promote clients.");
  }

  const { uid, email } = data;
  const password = "TempPass123!";

  try {
    await getAuth().createUser({
      uid,
      email,
      password,
    });

    await db.collection("users").doc(uid).set({
      email,
      role: "investor",
      completedSetup: false,
      createdAt: FieldValue.serverTimestamp(),
    });

    //update the status feild to "client" for the user document in investors
    await db.collection("investors").doc(uid).update({
      status: "client",
      tempPassword: password,
    });

    const msg = {
          to: email,
          from: "noreply.mailer.realest@gmail.com", // ✅ Replace with your verified sender
          subject: "Welcome to the Real Estate App!",
          html: `
            <h2>Your account has been created</h2>
            <p>We've created your account with the following credentials:</p>
            <ul>
              <li><strong>Email:</strong> ${email}</li>
              <li><strong>Temporary Password:</strong> ${password}</li>
            </ul>
            <p>Please log in and complete your setup.</p> `,
        };

    await sgMail.send(msg);


    return { success: true, uid, password };
  } catch (error: any) {
      console.error("Error promoting user",uid,"to client:", error.message, error?.response?.body || error);
      throw new HttpsError("internal", error.message || "Unknown error occurred");
    }
});

// New Firestore trigger for sending an email notification when a property is sent to a client
export const sendPropertyNotification = functions.firestore
  .document("investors/{investorId}/property_interactions/{interactionId}")
  .onWrite(async (
    change: functions.Change<FirebaseFirestore.DocumentSnapshot>,
    context: functions.EventContext
  ) => {
    // If the document is deleted, exit.
    if (!change.after.exists) {
      return null;
    }

    const afterData = change.after.data();
    const beforeData = change.before.exists ? change.before.data() : null;

    // Check if the new status is "sent"
    if (afterData?.status === "sent") {
      // If this is an update and status was already "sent" before, then do nothing.
      if (beforeData && beforeData.status === "sent") {
        return null;
      }

      // Get the investor's email from their document.
      const investorId = context.params.investorId;
      const db = getFirestore();
      const investorDocRef = db.collection("investors").doc(investorId);
      const investorDoc = await investorDocRef.get();

      if (!investorDoc.exists) {
        console.error(`Investor document for ${investorId} not found.`);
        return null;
      }

      const investorData = investorDoc.data();
      const investorEmail = investorData?.email;
      if (!investorEmail) {
        console.error(`Investor email not found for ${investorId}.`);
        return null;
      }

      // Compose and send the email via SendGrid
      const msg = {
        to: investorEmail,
        from: "noreply.mailer.realest@gmail.com", // Replace with your verified sender
        subject: "New Property Sent by Your Realtor",
        html: `
            <h3>You have a new property!</h3>
            <p>Your realtor has sent you a property to review. Log in to the app for more details.</p>
          `,
      };

      try {
        await sgMail.send(msg);
        console.log(`Email sent to ${investorEmail}`);
      } catch (error) {
        console.error("Error sending email:", error);
      }
    }
    return null;
  });
